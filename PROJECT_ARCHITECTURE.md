...
| :--- | :--- | :--- | :--- |
| **2026-09-03** | Mobile Statement Register Responsive Cards & Account Overview Table Isolation | Restored Customer Account Overview (`account-overview-table`) mobile styles 100% to its original compact table layout, resolving accidental inheritance from general `.statement-table` rules. Strictly isolated the new mobile responsive transaction cards using `:not(.account-overview-table)` so that only the Statement Register table receives the card layout (colored left-accent borders, full-width description, 3-column financial boxes, attachment chip, and actions toolbar). | `styles.css`, `app.js`, `khata.html`, `accounts-payable.html`, `PROJECT_ARCHITECTURE.md` |
