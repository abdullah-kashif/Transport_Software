...
2. **`bookings`** & **`booking_containers`**: Complete booking records and individual container line items (supports multi-container bookings).
   - Container rows also persist `quantity` and `unit_price` (nullable numeric columns). Run `supabase-container-pricing.sql` before deploying the invoice quantity/unit-price update. Existing rows remain null until their quantities and prices are re-entered and saved; historical per-row prices cannot be recovered reliably from the booking total.
   ...
   - Multiple broker payments persist as an ordered JSON array in `bookings.broker_entries`. Apply `supabase-booking-multi-brokers.sql` before deployment. The legacy single-broker columns remain populated for backward compatibility, and historical rows without JSON data load as one broker row.
...
Booking Form has a repeatable Trucker/Broker section immediately above Remarks. Every row stores Trucker/Broker, Container Ref, Amount, Payment/Cheque/IBFT reference, Payment Date, and Paid/Payable status. Each row's read-only Broker P&L is `Receivable Amount - that row's Amount`. The footer shows Total Broker Amount as the sum of all row amounts and job Net P&L as `Receivable Amount - Total Broker Amount`. Changes to container charges, taxes, withholding, detention, or any broker amount recalculate both values live. P&L values are derived on save/load and are not redundantly stored. Run `supabase-booking-multi-brokers.sql` before deployment; it adds the `broker_entries` JSONB column and safely retains the legacy single-broker columns. Existing records remain unchanged and continue to load through the legacy fallback. Broker Amount accepts zero or positive numbers with up to two decimal places.
...
| **2026-09-19** | Multi-Broker P&L Persistence & Ledger | Added repeatable broker rows with Container Ref and per-row Paid/Payable status. Broker P&L is calculated per row as Receivable Amount minus that row amount; Net P&L is calculated separately as Receivable Amount minus total broker amounts. Persisted complete rows in `bookings.broker_entries` with legacy fallback, and placed Broker P&L then Net P&L immediately before Remarks in the 44-column Booking Ledger. Apply `supabase-booking-multi-brokers.sql` before deployment. | `booking.html`, `app.js`, `styles.css`, `supabase-booking-multi-brokers.sql`, `PROJECT_ARCHITECTURE.md`, `AGENTS.md` |
| **2026-09-19** | Complete Booking Ledger Record View | Expanded Booking Ledger from 35 to 42 aligned columns so every saved Booking Form field is visible. Added selected Sales Tax Withholding; stacked per-container Quantity and Unit Price; Trucker/Broker, Amount, Payment/Cheque/IBFT, Broker Payment Date and Broker Payment Status. Retained Net P&L before Remarks and included the new values in General Filter search. Added Supabase save/load mapping plus `supabase-booking-sales-tax-withholding.sql` for the previously non-persisted withholding selection. | `booking.html`, `app.js`, `supabase-booking-sales-tax-withholding.sql`, `PROJECT_ARCHITECTURE.md`, `AGENTS.md` |
| **2026-09-19** | Booking Categories, Broker Payment Status & Job Net P&L | Added LCL/FCL categories and a broker Payment Status dropdown (Payable default, Paid). `brokerPaymentStatus` persists as nullable `bookings.broker_payment_status`; apply `supabase-booking-broker-status.sql` before deployment. Missing legacy status is treated as Payable. Paid/Payable tracks settlement and does not alter P&L. Booking Ledger shows Net P&L immediately before Remarks, calculated as Receivable Amount minus Broker Amount, with profit/loss colors and `-` when no broker amount exists. Broker section now has six desktop columns. | `booking.html`, `app.js`, `styles.css`, `supabase-booking-broker-status.sql`, `PROJECT_ARCHITECTURE.md`, `AGENTS.md` |
