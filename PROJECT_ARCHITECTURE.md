...
| :--- | :--- | :--- | :--- |
| **2026-09-03** | Mobile Statement Register Modern Responsive Cards Redesign | Completely redesigned the Statement Register (`statement-table`) mobile view in Accounts Receivable (`khata.html`) and Accounts Payable (`accounts-payable.html`). Replaced awkward, unsorted 2-column key-value grid with structured, modern transaction cards: left-side accent borders (red for Debit/Payable, green for Credit/Paid), top header row with date badge and colored transaction type pills, full-width prominent description headline, clean 3-column financial metrics strip (Debit, Credit, and Balance) with soft background tints and distinct color-coded amounts, attachment chips shown only when images exist (eliminating blank `IMAGE: -` rows), clean touch-friendly Actions toolbar shown only when actions exist (eliminating blank `ACTIONS: -` rows), empty-state card centering, and extra responsive scaling for narrow screens (<= 400px). | `styles.css`, `app.js`, `khata.html`, `accounts-payable.html`, `PROJECT_ARCHITECTURE.md` |
