...
Booking Form also has a separate optional Trucker/Broker payment section immediately above Remarks: `truckerBroker`, `brokerAmount`, `brokerPaymentDetails` (Payment/Cheque/IBFT reference), `brokerPaymentDate`, and read-only `brokerProfitLoss`. P&L = Receivable Amount - Amount, rounded to two decimals; a blank Amount leaves P&L blank. Changes to container charges, taxes, withholding or detention recalculate it. The normalizer derives P&L on save/load rather than storing a redundant database value. Apply `supabase-booking-broker.sql` before deployment to add nullable `trucker_broker`, `broker_amount`, `broker_payment_details`, and `broker_payment_date` columns to `bookings`; existing records remain unchanged. New details are included in the general ledger search and restored on Edit. Amount accepts zero or positive numbers with up to two decimal places. The responsive section has five desktop columns, two tablet columns, and one mobile column.
