...
2. **`bookings`**, **`booking_containers`** & **`booking_brokers`**: Complete booking records, individual container line items (supports multi-container bookings), and individual trucker/broker payment lines (supports multi-broker assignments).
   ...
   - Broker rows persist `trucker_broker`, `broker_amount`, `broker_payment_details`, `broker_payment_date`, and `sort_order`. Apply `supabase-booking-brokers-table.sql` on Supabase / PostgreSQL VPS before deployment.
   - For backwards compatibility, single-broker columns on `bookings` (`trucker_broker`, `broker_amount`, `broker_payment_details`, `broker_payment_date`) and `broker_lines` JSONB column are automatically maintained so legacy records and offline stores function without data loss.
...
Booking Form features a dynamic multi-row Trucker/Broker payment matrix (`.broker-block`) immediately above Remarks, mirroring the UX of container rows. Users can add multiple broker rows via "Add Broker Row". Each row includes: `truckerBroker`, `brokerAmount`, `brokerPaymentDetails` (Payment/Cheque/IBFT reference), `brokerPaymentDate`, read-only row-level `brokerProfitLoss`, and a `Remove` button. An integrated summary bar dynamically aggregates `Total Broker Amount` ($\sum \text{Broker Amounts}$) and `Net P&L` ($\text{Receivable Amount} - \text{Total Broker Amount}$). Rows are normalized into `booking.brokerLines` with primary fields mirrored to top-level properties for 100% backwards compatibility. Apply `supabase-booking-broker-lines.sql` before deployment to persist multi-row broker lists in Supabase (`broker_lines jsonb`). All broker details across all rows are indexed in the general ledger search and restored during Edit. Amount validation checks for non-negative numbers with up to two decimal places across all rows. Desktop uses a 6-column grid; mobile/tablet collapses into readable stacked rows.
...
P&L example: container freight 200 with detention 100 and the existing 15% sales tax / 7% income withholding / 20% sales-tax withholding produces receivable 307.90. Broker Amount 100 gives P&L +207.90.
