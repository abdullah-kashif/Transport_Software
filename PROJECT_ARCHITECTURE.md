...
| :--- | :--- | :--- | :--- |
| **2026-09-03** | Mobile Statement Register Responsive Cards, Overview Isolation & Share Sheet Cancellation Fix | (1) Restored Customer Account Overview (`account-overview-table`) mobile styles 100% to original compact table layout by strictly isolating Statement Register card styles with `:not(.account-overview-table)`. (2) Fixed Web Share API cancellation behavior in `shareStatementOnWhatsapp` in `app.js`: when a user cancels or dismisses the mobile native share sheet, `AbortError` / `NotAllowedError` is cleanly caught and execution stops without mistakenly redirecting the user to WhatsApp. | `styles.css`, `app.js`, `khata.html`, `accounts-payable.html`, `PROJECT_ARCHITECTURE.md` |
